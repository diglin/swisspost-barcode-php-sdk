...
/**
 * This class stands for ReadLabelLayouts StructType
 * @subpackage Structs
 * @date 2019/01/11
 * @codeVersion 1.0.0
 */
class ReadLabelLayouts extends AbstractStructBase
{
    /**
     * The Language
     * @var string
    ...
    public $PRZL;
    /**
     * Constructor method for ReadLabelLayouts
     * @uses ReadLabelLayouts::setLanguage()
     * @uses ReadLabelLayouts::setPRZL()
    ...
    public function setPRZL(array $pRZL = array())
    {
        ...
        foreach ($pRZL as $readLabelLayoutsPRZLItem) {
            // validation for constraint: itemType
            if (!is_string($readLabelLayoutsPRZLItem)) {
                throw new \InvalidArgumentException(sprintf('The PRZL property can only contain items of string, "%s" given', is_object($readLabelLayoutsPRZLItem) ? get_class($readLabelLayoutsPRZLItem) : gettype($readLabelLayoutsPRZLItem)), __LINE__);
            }
        }
        $this->PRZL = $pRZL;
        return $this;
